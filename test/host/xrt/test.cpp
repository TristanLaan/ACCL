...
void test_copy(ACCL::ACCL &accl, options_t &options) {
  ...
  auto op_buf = std::unique_ptr<Buffer<float>>(
...
void test_combine(ACCL::ACCL &accl, options_t &options) {
  ...
  unsigned int count = options.count;
  auto op_buf1 = std::unique_ptr<Buffer<float>>(
      accl.create_buffer<float>(count, dataType::float32));
  auto op_buf2 = std::unique_ptr<Buffer<float>>(
      accl.create_buffer<float>(count, dataType::float32));
  auto res_buf = std::unique_ptr<Buffer<float>>(
      accl.create_buffer<float>(count, dataType::float32));
...
void test_gather(ACCL::ACCL &accl, options_t &options, int root) {
  ...
  auto op_buf = std::unique_ptr<Buffer<float>>(accl.create_buffer(
      host_op_buf.get() + count * rank, count, dataType::float32));
  std::unique_ptr<ACCL::Buffer<float>> res_buf;
  if (rank == root) {
    res_buf = std::unique_ptr<Buffer<float>>(
        accl.create_buffer<float>(count * size, dataType::float32));
  } else {
